...
export async function loadFamilyBundle(db: Db, userId: string): Promise<FamilyBundle> {
  ...
  const events: CalendarEvent[] = (eventsRes.data ?? []).map((e: any) => {
    ...
    // Presentation metadata from the calendar's own appearance settings.
    source_color: (source?.color ?? null) as MemberColor | null,
    source_icon: source?.display_icon ?? null,
