...
>(function MonthScrollView(
  ...
  const scrollToMonth = useCallback((month: Date, behavior: ScrollBehavior = "smooth") => {
    ...
    const section = sectionRefs.current.get(format(startOfMonth(month), "yyyy-MM"));
  ...
  useEffect(() => {
    ...
    const evaluate = () => {
      ...
      for (const month of months) {
        const section = sectionRefs.current.get(key(month));
        if (!section) continue;
        if (section.offsetTop <= top) active = month;
        else break;
      }
