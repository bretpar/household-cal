...
describe('email summary calendar eligibility', () => {
  ...
  it('keeps only explicitly selectable, active, non-coverage calendars', () => {
    expect(emailSelectableCalendars([google, localReal, bucket, coverage, inactive]).map((s) => s.id)).toEqual(['g', 'l'])
  })
  ...
  it('coverage background can never qualify', () => {
    expect(isEmailSelectableCalendar(coverage)).toBe(false)
  })
})
